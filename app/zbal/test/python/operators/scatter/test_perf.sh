...
if [ "$OP" = "0" ]; then
  ...
  for param in "${params[@]}"; do
      bash test_zbal_scatter.sh "$param"
  done
else
  # zbal
  for param in "${params[@]}"; do
      echo "zbal scatter data_len:${param}" >> ../perf.txt
      dir_path="./profiling.zbal_${param}"
      fn_analyze_perf "${dir_path}" "ZBALScatterInner" >> ../perf.txt
  done

  # hccl
  for param in "${params[@]}"; do
      echo "hccl scatter data_len:${param}" >> ../perf.txt
      dir_path="./profiling.hccl_${param}"
      fn_analyze_perf "${dir_path}" "hcom_scatter" >> ../perf.txt
  done
fi