...
if [ "$OP" = "0" ]; then
  ...
  for param in "${params[@]}"; do
      bash test_zbal_broadcast.sh "$param"
  done
else
  # zbal
  for param in "${params[@]}"; do
      echo "zbal broadcast data_len:${param}" >> ../perf.txt
      dir_path="./profiling.zbal_${param}"
      fn_analyze_perf "${dir_path}" "ZBALBroadcastInner" >> ../perf.txt
  done

  # hccl
  for param in "${params[@]}"; do
      echo "hccl broadcast data_len:${param}" >> ../perf.txt
      dir_path="./profiling.hccl_${param}"
      fn_analyze_perf "${dir_path}" "hcom_broad" >> ../perf.txt
  done
fi