...
if [ "$OP" = "0" ]; then
  ...
  for param in "${params[@]}"; do
      bash test_zbal_allreduce.sh "$param"
  done
else
  # zbal
  for param in "${params[@]}"; do
      echo "zbal allreduce data_len:${param}" >> ../perf.txt
      dir_path="./profiling.zbal_${param}"
      fn_analyze_perf "${dir_path}" "ZeroBuffAllReduce" >> ../perf.txt
  done

  # hccl
  for param in "${params[@]}"; do
      echo "hccl allreduce data_len:${param}" >> ../perf.txt
      dir_path="./profiling.hccl_${param}"
      fn_analyze_perf "${dir_path}" "hcom_allReduce__" >> ../perf.txt
  done
fi
