...
if [ "$OP" = "0" ]; then
  ...
  for param in "${params[@]}"; do
      bash test_zbal_p2p.sh "$param"
  done
else
  # zbal
  for param in "${params[@]}"; do
      echo "zbal p2p data_len:${param}" >> ../perf.txt
      dir_path="./profiling.zbal_${param}"
      fn_analyze_perf "${dir_path}" "ZBALSendInner" >> ../perf.txt
      fn_analyze_perf "${dir_path}" "ZBALRecvInner" >> ../perf.txt
  done

  # hccl
  for param in "${params[@]}"; do
      echo "hccl p2p data_len:${param}" >> ../perf.txt
      dir_path="./profiling.hccl_${param}"
      fn_analyze_perf "${dir_path}" "hcom_send__" >> ../perf.txt
      fn_analyze_perf "${dir_path}" "hcom_receive__" >> ../perf.txt
  done
fi