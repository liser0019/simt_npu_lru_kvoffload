...
if [ "$OP" = "0" ]; then
  ...
  for param in "${params[@]}"; do
      bash test_zbal_allgather.sh "$param"
  done
else
  # zbal
  for param in "${params[@]}"; do
      echo "zbal allgather data_len:${param}" >> ../perf.txt
      dir_path="./profiling.zbal_${param}"
      fn_analyze_perf "${dir_path}" "ZBALAllGatherInner" >> ../perf.txt
  done

  # hccl
  for param in "${params[@]}"; do
      echo "hccl allgather data_len:${param}" >> ../perf.txt
      dir_path="./profiling.hccl_${param}"
      fn_analyze_perf "${dir_path}" "hcom_allGather__" >> ../perf.txt
  done
fi
