...
This production extension intentionally builds one current optimized
implementation per operator, plus generic SIMT fallbacks for unsupported
shapes.  The repository still retains the pre-existing generic AIV
Compact/ResidentAddrs source files from the ``release_kv_miss`` baseline so
this feature branch does not delete unrelated framework code; they are not in
the production run-package build list below.
...
Historical V1/V2, mixed-plan, thread-sweep, MTE3 and scalar-SIMT SparseCopy
experiment sources are not included in this feature branch. The pre-cleanup
research snapshot is not required to build or use the production extension.
